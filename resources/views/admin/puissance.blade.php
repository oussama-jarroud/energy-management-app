    ...
    <div class="px-20 py-14">
        <div class="max-w-5xl mx-auto">
            <div class="bg-white rounded-lg shadow-lg p-6">
                <h2 class="text-lg font-semibold mb-4">Consommation de Puissance (Mois)</h2>
                <div class="flex justify-center">
                    <canvas id="puissanceChart" width="900" height="450"></canvas>
                </div>
                <div class="flex justify-between mt-4">
                    <form method="GET" action="{{ route('admin.puissance') }}">
                        <div class="flex">
                            <div>
                                <label for="fromDatep" class="text-sm font-semibold text-gray-600">Du:</label>
                                <input type="date" name="fromDatep" id="fromDatep" class="ml-2 border border-gray-300 rounded-md px-3 py-1" value="{{ request('fromDatep') }}">
                            </div>
                            <div class="ml-4">
                                <label for="toDatep" class="text-sm font-semibold text-gray-600">Au:</label>
                                <input type="date" name="toDatep" id="toDatep" class="ml-2 border border-gray-300 rounded-md px-3 py-1" value="{{ request('toDatep') }}">
                            </div>
                            <div class="ml-4">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function getCurrentMonthData(puissances) {
            const currentDate = new Date();
            const currentMonthData = puissances.filter(puissance => {
                const puissanceDate = new Date(puissance.created_at);
                return puissanceDate.getMonth() === currentDate.getMonth() && puissanceDate.getFullYear() === currentDate.getFullYear();
            });
            return currentMonthData;
        }

        // Initial data from Laravel
        const puissances = @json($puissances);

        // Generate labels and data for the current month
        const labels = getCurrentMonthLabels();
        const currentMonthData = getCurrentMonthData(puissances);
        
        const puissanceAtelieData = labels.map(day => {
            const dataPoint = currentMonthData.find(puissance => new Date(puissance.created_at).getDate() === day);
            return dataPoint ? dataPoint.puissance_atelie : null;
        });
        
        const puissanceAdminData = labels.map(day => {
            const dataPoint = currentMonthData.find(puissance => new Date(puissance.created_at).getDate() === day);
            return dataPoint ? dataPoint.puissance_admin : null;
        });
        ...
        const puissanceData = {
            ...
            datasets: [
                {
                    label: 'Puissance Atelie',
                    data: puissanceAtelieData,
                    borderColor: '#4299E1',
                    borderWidth: 2,
                    pointRadius: 3,
                    spanGaps: true,
            ...
            options: {
                scales: {
                    y: {
                        beginAtZero: true,
                        min: 0,
                        max: 440 // Adjust max value as needed
                    },
                    x: {
                        grid: {
                            display: false
        ...
        var puissanceChart = new Chart(
            document.getElementById('puissanceChart'),
            puissanceConfig
        );
        ...
        function updateChart() {
            ...
            fetch(`/puissance-data?fromDate=${fromDate}&toDate=${toDate}`)
                .then(response => response.json())
                .then(data => {
                    // Update Chart.js chart with new data
                    puissanceChart.data.labels = data.labels;
                    puissanceChart.data.datasets[0].data = data.puissanceAtelieData;
                    puissanceChart.data.datasets[1].data = data.puissanceAdminData;
                    puissanceChart.data.datasets[2].data = data.usineData;
                    puissanceChart.data.datasets[3].data = data.magasinData;
